...
<input type="hidden" name="func" value="<?=$func?>" />
    ...
    while($rnn=$db->fetch($qnn)){
        $dem1++;
        $qnn1=$db->select("post_file_lang","file_id='".$id."' and lang_id='".$rnn['code']."'","order by id");
        $rnn1=$db->fetch($qnn1);
    ?>
        ...
        <div class="form-group">
            <label class="control-label">Tài liệu [<?=$rnn['code']?>]</label>
            <textarea name="txt_noi_dung[<?=$rnn['code']?>]" class="form-control" rows="2"><?=$rnn1['noi_dung']?></textarea>
            ...
            <table class="table table-striped">
                <tbody>
                    <tr>
                        ...
                        <?
                        global $db;
                        $q1=$db->select("post_file","id='".$id."'","");
                        $r1=$db->fetch($q1);
                        {
                        ?>
                        <p style="text-align: center;background: #fff;margin: 10px 0;">
                            <a class="link_file" href="<?=$domain?>/uploads/<?=$r1['dir'].$r1['file']?>" target="_blank"><?=$r1['file']?></a>
                        </p>
                        <?}?>
                        </td>
                        <td>
...
{
    global $db;	
    $r2 = $db->select("post_file_menu"," ","order by id asc");
?>
