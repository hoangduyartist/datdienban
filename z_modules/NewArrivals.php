<div class="bg-while">
  <?php
  echo albumBannerSlide(5);
  $page       =   $page + 0;
  $perpage    =   100;
  $r_all      =   $db->selectpost(
    "hien_thi=1 and is_new='1' and post_type='sanpham_detail' and lang_id='" . $lang_code . "'",
    ""
  );
  ...
  <div class="mt30">
    <div class="container">
      <h1 class="title-pages detail">NEW ARRIVALS</h1>
      <?php
      $q = $db->selectpost("hien_thi=1 and is_new='1' and post_type='sanpham_detail' and lang_id='" . $lang_code . "'", "order by post.time_edit LIMIT " . $min . ", " . $max);
      if ($db->num_rows($q) != 0) {
      ?>
        <div class="row">
          <?php
